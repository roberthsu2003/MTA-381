...
A：請問第1行應該填入下列哪段敘述？ ① for i in range(len(score) - 1):
...
01 # 顯示 3 X 10 乘法表 02 def print_tables():
...
```
請利用下列敘述區段的代碼(可省略或重複使用)，組合成一個顯示 1 ~ 50中間所有質數的程式。
...
```
請設計一個行動電話號碼檢查程式，電話號碼要符合下列條件：
• 格式必須為 0000-000-000，只由數字和破折號-組成。 • 格式正確就顯示True，否則就顯示False。
 請回答以下問題來完成程式：
