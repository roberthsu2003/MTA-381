...
w -> 打開一個文件只用於寫入。如果該文件已存在則打開文件，並從開頭開始编辑，即原有内容会被删除。如果該文件不存在，建立新文件。

r -> 以只讀方式打開文件。文件的指针將會放在文件的開頭。這是預設模式。
...
③ input('請輸入成績(0 ~ 100)，停止時請輸入0：')
④ input('請輸入成績(0 ~ 100)，停止時請輸入-1：')
...
```
8.下列為讀取資料檔並將結果依照格式列印的函式，函式的功能如下
