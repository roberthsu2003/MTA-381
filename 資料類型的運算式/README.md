...
```
11下列函式可以將傳入的字串，以相反的順序重組後傳回新字串，請回答下列問題來完成程式碼。
...
```
12以下程式碼是計算今天遊戲過關數，請問第03 行應該填入下列哪個敘述？
...
```
13如果word = 'ABCDEFGHIJKLMNOPQRSTUVWXYZ' 時，請用代碼填入下列分割操作的結果：
...
```
15books 串列(清單)包含 300 本書的編號，如要取得第2個編號開始到最後，而且間隔1個編號的串列(如：2, 4, 6…)，請問可以使用下列哪個指令？
...
```
16下列程式碼中，x是使用者輸入的整數，y 等於 x 乘以 -1，然後再平方。
...
```
17下列為傳回折扣值的函式，如果傳入值female(女性)、kid(兒童)其中一個為真，就獲得 5% 的折扣。
請問第 03 行應該填入下列哪個敘述？
...
```
18請使用代碼填入下列運算式的結果。① True ② False ③ 3 ④ None 
...
```
23下列為計算字串中指定字母數量的程式碼，請回答以下問題來完成程式
...
```
25下列為一個除法函式，函式中檢查分母不能為零，以及需要兩個數值。
請回答以下問題來完成程式。
...
```
26如果x=13、y=5，請使用代碼填入下列是哪個運算式的執行結果：
