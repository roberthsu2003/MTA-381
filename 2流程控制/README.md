...
```
2下列為根據最低年齡制定電影分級的函式，假設電影分級的規則如下：
...
```
6下列為猜1到20亂數整數的程式碼，猜對就結束程式, 猜錯就顯示訊息最多可以猜4次。
請用代碼填入以下問題來完成程式：
...
```
9下列為由score串列讀取成績，並調整成績的程式碼。調整成績的規則如下：
...
```
10下列為顯示3 ~ 10乘法表函式的程式碼，請回答以下問題來完成程式：
...
```
11請利用下列敘述區段的代碼(可省略或重複使用)，組合成一個顯示 1 ~ 50中間所有質數的程式。
